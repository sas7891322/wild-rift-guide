...
const report = read('assets/data/calibration-7.2e-v94.json');
expect(patchData.version === '7.2e' && patchData.siteVersion === 'v94', 'Current patch mismatch');
...
for(const change of report.tierChanges) {
 const profile=change.profileId.startsWith('aram:')?aram.heroes.find(x=>x.id===change.profileId.slice(5)):profileById.get(change.profileId);
 expect(profile.tier===change.to, `Tier mismatch ${change.profileId}`);
}
...
console.log(JSON.stringify({version:'v94',riftProfiles:202,aramProfiles:141,aaaProfiles:140,structureAndReferences:'PASS',protectedFiles:'PASS',officialDelta:'PASS'},null,2));

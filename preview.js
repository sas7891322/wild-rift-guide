...
fetch("UPDATE_FILES/assets/data/items.json").then(r=>r.json()).then(j=>{data=j;init()});
function init(){
...
function render(){
  ...
  $("#grid").innerHTML=items.length?items.map((i,n)=>`<button class="item-card" data-index="${data.categories[active].indexOf(i)}"><img src="UPDATE_FILES/${i.icon}" alt="${i.name}"><h3>${i.name}</h3><div class="price">${i.price} 金幣</div><div class="mini-stats">${i.stats.slice(0,3).join("<br>")}</div></button>`).join(""):`<div class="empty">找不到符合條件的裝備。</div>`;
  $("#grid").onclick=e=>{const c=e.target.closest("[data-index]");if(c)openModal(data.categories[active][+c.dataset.index])};
}
function openModal(i){
  $("#detailIcon").src="UPDATE_FILES/"+i.icon;$("#detailIcon").alt=i.name;$("#detailCategory").textContent=labels[active]+"・中階";
